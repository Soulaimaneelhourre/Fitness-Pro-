...
if(isset($_POST['signup'])){
    ...
    if($password !== $cpassword){
        $errors['password'] = "The Passwords Doesn't Matche";
    }
    ...
    if(count($errors) === 0){
        ...
        $confirmation_code = rand(999999, 111111);
        $status = "NOt verified";
        $insert_data = "INSERT INTO usertable (name, email, password, code, status, phone )
                        values('$name','$email','$encrypt_password','223344','$status','$phone')";
        $data_check = mysqli_query($conect,$insert_data);
        if($data_check){
            $subject= " email verification code ! ";
            $message= "your verification code id $confirmation_code";
            ...
            $errors['db_erroe']= "failed inserting date into database!";
        }
    }
}
